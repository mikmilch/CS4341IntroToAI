...
def expand_queue(queue, newNodesToAddToQueue, problem, search):
    ...
    if search == SearchEnum.DEPTH_FIRST_SEARCH:
        ...
        """
        Include your solution for Breadth-1st search here. Hint: 1 line of code (a call to a function provided) suffices.
        """
        ...
        """
        'TODO: Is <= or < ?'
        if (len(newNodesToAddToQueue) != 0) and (len(newNodesToAddToQueue[0].path) <= lengthLimit):
            queue.insertNodesAtFront(newNodesToAddToQueue)
        ...
        if(iterativeLimit == 0): 
            ...
            Include your solution for Depth-limited here. Hint: 1 line of code (a call to a function provided) sufficies, but you can have more lines if you need to.
            """
            if (len(newNodesToAddToQueue) != 0) and ((len(newNodesToAddToQueue[0].path)-1) <= iterativeLimit):
                queue.insertNodesAtFront(newNodesToAddToQueue)
